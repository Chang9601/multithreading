...
write_to_file(void *arg)
{
  ...
  while (1)
  {
    ...
    fflush(fp);
    sleep(1);
    // pthread_testcancel() 함수를 호출할 때, 취소 요청이 대기 중이고 취소가 비활성화되지 않았다면 스레드는 취소된다. 그러나 취소가 비활성화된 경우 pthread_testcancel() 함수 호출은 효과가 없다.
    // 즉, 스레드가 pthread_testcancel() 함수를 실행할 때마다 보류 중인 취소 신호가 있는지 확인하고 보류 중인 취소 신호가 있다면, 스레드는 그 줄에서 바로 취소된다.
    // 함수 호출에서 불변성 문제가 발생하지 않는데 함수가 완전히 실행되고 실행이 구현 내의 무작위 지점에서 취소되지 않기 때문이다.
    pthread_testcancel();
  }
